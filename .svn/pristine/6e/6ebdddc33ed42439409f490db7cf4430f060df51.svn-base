...
$.validator.addMethod("notequal", function(value, element) {
	 return $('#toEmailId').val() != $('#byEmailId').val()}, 
...
$.validator.addMethod("validateUserEmail", function(value, element){
		    var inputElem = $('#emailId').val();
		    var userPkId = $("#userPkId").val();
		    $.ajax({
		        type: "POST",
		        url: 'validateUserDuplicateEmail.html',
		        data: {
		        	email:inputElem,
		        	userPkId:userPkId
		        },
...
$.validator.addMethod( "quantity", function( value, element ) {
	return this.optional( element ) || /^0*[1-9][0-9]*(\.[0-9]+)?|0+\.[0-9]*[1-9][0-9]*$/.test( value );
}, "Please Enter valid quantity" );


$.validator.addMethod( "amountPaid", function( value, element ) {
	var amountBalance=$("#duesAmount").val();
	var blanceAmount=parseFloat(amountBalance);
	var paidAmount=parseFloat(value);
	/*return isNaN(value);
   "Please Enter the valid amount";*/
	if(amountBalance !='' && amountBalance !=undefined){
	return blanceAmount>=paidAmount;
	}
	
}, "Paid Amount must be less then blance amount");
